...
export default function GuidedWorkflowOverlay({ workflow, setCurrentPage, onClose, highlightPage }) {
  ...
  useEffect(() => {
    if (step?.targetPage && setCurrentPage) {
      setCurrentPage(step.targetPage)
    }
  }, [stepIdx, workflow?.id, step?.targetPage, setCurrentPage])

  if (!workflow) return null

  const isLast = stepIdx >= workflow.steps.length - 1
  const activeHighlight = highlightPage || step?.targetPage
  ...
  if (minimized) {
    return (
      <button
        ...
        className="fixed bottom-20 right-4 z-50 px-4 py-2 bg-brand text-white rounded-full shadow-lg text-sm flex items-center gap-2"
      >
  ...
  return (
    <>
      {activeHighlight && (
        <style>{`[data-nav-id="${activeHighlight}"] { outline: 2px solid var(--color-brand-light, #6b8f71); outline-offset: 2px; }`}</style>
      )}
      <div className="fixed bottom-20 right-4 z-50 w-[min(360px,calc(100vw-2rem))] card p-4 shadow-xl border-2 border-brand/30 no-print">
        <div className="flex justify-between items-start gap-2 mb-2">
          <div>
            <p className="text-xs text-brand font-medium uppercase tracking-wide">Guided workflow</p>
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          <button
            ...
            className={`px-3 py-1.5 ${btnPrimary} text-xs flex items-center gap-1`}
          >
